...
###############################################################################
#必需参数：AE集群所在地域，请按实际填写!
AWS_DEFAULT_REGION="us-east-1"

#必需参数：AE主机所在VPC的ID
VPC_ID="vpc-01799f2a4f56d7482"

#必需参数：AE主机实际所在子网，十分重要！请从AWS控制台确认后填写！下方还有子网信息收集应包含AE 主机子网
#后续流程会进行子网打标并让容器实例会绑定相同子网！
AE_HOSTS_SUBNET_ID="subnet-0d71db2e9eddb8e56"

#必需参数：AE主机绑定安全组，十分重要！将用于AE主机与EKS容器集群内网互联互通，请从AWS控制台确认后填写！
AE_HOSTS_SG_ID="sg-092b5d2b62758a1fc"

#必需参数:可用区1
ZONE_1="us-east-1a"
#可用区1下public subnet id
PUBLIC_SUBNET_ZONE_1="subnet-04dbd46e836adc95b"

#必需参数:可用区2
ZONE_2="us-east-1d"
#可用区2下public subnet id
PUBLIC_SUBNET_ZONE_2="subnet-0d71db2e9eddb8e56"
...
iam_self_check() {
  ...
  if policies="$(aws iam list-attached-role-policies --role-name "${role_name}" --output json 2>/dev/null)"; then
    ...
    yellow "提示：此处只能读取到 attached managed policy，如果最终结果不符合预期，请检查是否存在inline policy 与账户 SCP 边界限制。"
  else
...
#     3) 所有子网(2可用区public + AE主机子网)存在,且都属于该 VPC
