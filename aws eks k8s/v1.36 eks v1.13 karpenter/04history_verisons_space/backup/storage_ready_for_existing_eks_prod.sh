...
#   与 build_eks_v1.36_deploy_prod.sh 的区别：
#     - 那个是"新建集群连带存储";本脚本只对【已存在集群】补存储,
#       绝不创建集群/节点组,不改网络放行/打标签,不装 Karpenter。
#   完成内容：
...
#
# 【同源说明】存储相关函数(ensure_addon/addon_wait_active/efs_*/subnet_az/
#   efs_ensure_mount_targets/apply_sc_*/claim_default_sc)与工具安装/打印基础设施,
#   与 build_eks_v1.36_deploy_prod.sh 阶段5 同源。任一处修改需【两个文件同步】。
#
# 前置：管理员需从 AWS 控制台确认并在下方配置区手填 4 项(region/VPC/集群名/AE子网);
#       其余(cluster SG / ShareNode SG / 集群子网)由脚本解析集群自动获取。
...
# AWS 托管策略 ARN。EBS 用 V2(支持 gp3/快照);中国区(aws-cn)/GovCloud 需自行确认该托管策略可用。
EBS_CSI_POLICY_ARN="${EBS_CSI_POLICY_ARN:-arn:${AWS_PARTITION}:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicyV2}"
...
# 端到端实测开关(true 时起 PVC+Pod 实测 EBS/EFS 动态供给;默认关)
STORAGE_E2E_TEST="${STORAGE_E2E_TEST:-false}"
...
补存储所需 AWS 权限(供管理员参考,权限不足会在对应步骤自然报错)：
  ...
  复用托管策略 AmazonEBSCSIDriverPolicyV2 / AmazonEFSCSIDriverPolicy (由 eksctl 关联)
TIP
}
...
ensure_addon() {
  ...
  if [ "${st}" == "NONE" ]; then
    ...
    local vflag=()
    [ -n "${ver}" ] && vflag=(--version "${ver}")
    # --force: 若集群已存在自管版本则接管配置;pod-identity-associations 让 eksctl 自动建角色+关联
    eksctl create addon --cluster "${CLUSTER_NAME}" --name "${name}" "${vflag[@]}" \
      --pod-identity-associations "serviceAccountName=${sa},permissionPolicyARNs=${policy}" --force
  else
...
efs_ensure_mount_targets() {
  ...
  # 等所有挂载目标进入 available:挂载目标 available 前,EFS 的 DNS 名无法解析出本 AZ 私网 IP,
  # 节点侧 mount -t efs 会 DNS 解析失败并转而调 API 兜底。故此处必须等齐。
...
parameters:
  type: ${SC_DISK_TYPE}
  csi.storage.k8s.io/fstype: ${SC_DISK_FSTYPE}
EOF
...
verify_storage() {
  ...
  # 端到端实测(可选,STORAGE_E2E_TEST=true 开启)
  if [ "${STORAGE_E2E_TEST}" == "true" ]; then
    echo "*** 存储端到端实测(STORAGE_E2E_TEST=true): 起 PVC+Pod 验证动态供给 ***"
    verify_storage_e2e "${SC_DISK_NAME}" "ebs" || note_attention "EBS(${SC_DISK_NAME}) 端到端实测未通过,请手动排查。"
    verify_storage_e2e "${SC_NFS_NAME}" "efs" || note_attention "EFS(${SC_NFS_NAME}) 端到端实测未通过,请手动排查。"
  else
    echo "(存储端到端实测已跳过;如需实测请以 STORAGE_E2E_TEST=true 重跑)"
  fi
}

# 端到端实测单个 SC:建 PVC + 挂载 Pod,等 Pod Ready(EBS 用 WaitForFirstConsumer,Pod 起来才触发供给)。
# 入参: sc名  标记(ebs/efs,仅用于命名与提示)。测完清理。返回 0 通过 / 非0 失败。
verify_storage_e2e() {
  local sc="$1" tag="$2" ns="${VERIFY_NAMESPACE}"
  local pvc="e2e-${tag}-pvc" pod="e2e-${tag}-pod" ok=false t=0 phase
  kubectl get ns "${ns}" >/dev/null 2>&1 || kubectl create ns "${ns}" >/dev/null 2>&1
  local img="${VERIFY_NGINX_IMAGE}"
  [[ "${AWS_DEFAULT_REGION}" =~ ^cn- ]] && img="${VERIFY_NGINX_IMAGE_CN}"
...
spec:
  ...
  - name: vol
    persistentVolumeClaim:
      claimName: ${pvc}
EOF
  echo "等待 ${tag} 测试 Pod 就绪(最多 180s)..."
  while [ ${t} -lt 180 ]; do
    phase="$(kubectl get pod "${pod}" -n "${ns}" -o jsonpath='{.status.phase}' 2>/dev/null || true)"
    if [ "${phase}" == "Running" ]; then
      green "${tag} 端到端实测通过：PVC 绑定且 Pod Running(${sc} 动态供给成功)。"
      ok=true
      break
    fi
    echo "等待 ${tag} Pod...(状态: ${phase:-未知}, 已等待 ${t}s)"
    sleep 6
    t=$((t + 6))
  done
  # 清理测试资源
...
阶段4: 存储就绪性校验(SC / CSI 驱动 / EFS 挂载目标 + 可选端到端实测)
