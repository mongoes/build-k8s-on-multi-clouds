...
# =============================================================================
# build_eks_v1.36.sh
# 执行本脚本的云主机 IAM 角色需具备 AdministratorAccess(或等价创建EKS权限)。
...
AE_HOSTS_SUBNET_ID="subnet-aacac4e3"
#AE主机绑定安全组，十分重要！将用于AE主机与EKS容器集群内网互联互通，请从AWS控制台确认后填写！
AE_HOSTS_SG_ID="sg-0467cf502fd8278f1"

#可用区1
ZONE_1="us-east-1a"
#可用区1下public subnet id
PUBLIC_SUBNET_ZONE_1="subnet-aacac4e3"
#可用区1下private subnet id
PRIVATE_SUBNET_ZONE_1="subnet-0c01956bf3c1f24d1"

#可用区2
ZONE_2="us-east-1b"
#可用区2下public subnet id
PUBLIC_SUBNET_ZONE_2="subnet-0c01956bf3c1f24d1"
#可用区2下private subnet id
PRIVATE_SUBNET_ZONE_2="subnet-0ed7db55eebe7060d"
...
managedNodeGroups:
  - name: ${CLUSTER_NAME}-ng
    ...
    # 声明式赋予节点实例角色 EFS 权限(eksctl withAddonPolicies.efs)：
    #   efs:true 会为 NodeInstanceRole 内联附加 PolicyEFS(elasticfilesystem:*，含
    #   DescribeMountTargets)+ PolicyEFSEC2(网卡类)。EFS 挂载在节点上进行，DNS 兜底需
    #   DescribeMountTargets，故让托管节点组出生即带权，取代阶段5事后 attach(消除
    #   is_done 守卫吞掉/硬编码节点组名/老集群补不上三坑)。
    #   注意：withAddonPolicies 不影响 eksctl 自动附加默认托管策略(WorkerNode/CNI/ECR)，
    #   兼容性优于 attachPolicyARNs(后者一经使用即须手工重列全部默认策略)。
    #   来源已核实：eksctl iam_helper.go:133 / statement.go:414 + AWS 官方 eksctl 文档。
    iam:
      withAddonPolicies:
        efs: true
...
efs_ensure_mount_targets() {
  ...
  # 挂载目标 available 前,EFS 的 DNS 名(fs-xxx.efs.<region>.amazonaws.com)无法解析出本 AZ 私网 IP,
  # 节点侧 mount -t efs 会 DNS 解析失败并转而调 API 兜底,故此处必须等齐再往下。
...
#       ★ 该角色已在 ClusterConfig managedNodeGroups[].iam.withAddonPolicies.efs:true 处声明式带权,
#         此处 attach 仅作幂等兜底(存量集群/旧配置重跑时补齐),新建集群通常已冗余但无害。
#     - KarpenterNodeRole-<集群名>(Karpenter 主力节点,阶段2a CloudFormation 建,名字固定)
#       ★ Karpenter 节点角色不在 eksctl ClusterConfig 内,声明式无法覆盖,本函数是其唯一赋权途径,不可删。
