...
build_nodepool_for_business() {
  ...
  #预置变量子网标签value，取EKS集群名，默认不需要手动修改,除非当前你处于EKS版本升级场景时需要将子网value修改为你新定义的值
  subnetSelectorValue=${CLUSTER_NAME}

  #如有先备份已有节点组的yaml配置,并检查已有节点组的名称、机型、子网标签key/value、安全组标签key/value，与数数规范对比；当不一致时询问用户是否需要复用老节点组配置
  nodepool_records=$(kubectl get nodepool | grep -v 'NAME' | wc -l 2>/dev/null)
  if [ ${nodepool_records} -gt 0 ]; then
    #如果检测到已有karpenter节点组记录，先备份留档
    echo "检测到当前EKS环境中存在如下 ${nodepool_records} 个节点组,正在检查已有节点组的关键配置是否符合规范,请稍等~"
  ...
  read -p "请为每个节点组指定付费类型（支持on-demand spot,顺序需与节点组名称一一对应,多付费类型之间空格分隔）: " input_nodepool_billing_mode
  echo ""
  # 转换为数组
  ...
  #验证输入的付费类型是否在支持范围内
  for billing_mode in "${NODEPOOL_BILLING_MODE[@]}"; do
    if [[ "$billing_mode" != "on-demand" && "$billing_mode" != "spot" ]]; then
      log_error "错误：无效付费类型 '$billing_mode'，支持on-demand spot，请重新输入！"
      exit 1
    fi
  done
...
EOF
  ...
  if ! $POD_READY; then
    log_error "测试结论：异常！Pod未在${TIMEOUT}秒内启动，请kubectl describe pods -n $NAMESPACE -l app=nginx-test人工排查，常见原因: "
    echo -e "\e[31m\e[1m1. K8S环境节点组 ${NODEPOOL_NAME} 不通外网/镜像仓库导致镜像拉取失败\n2. K8S无实例可调度 \e[0m"
    exit 1
  fi
