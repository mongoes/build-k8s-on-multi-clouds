...
# 必需参数1:EKS集群名,可自定义，1.32版本EKS默认命名为eks-shushu-ta-v1-32
CLUSTER_NAME="eks-agenticengine"
#必需参数2：EKS所在地域，这里假设是us-east-1
AWS_DEFAULT_REGION="us-east-1"

#########################################
#其他参数已经由数数团队预置，请保持默认不需修改
KARPENTER_VERSION="1.11"
...
create_karpenter_iamserviceaccount() {
  ...
  if ! aws iam get-role --role-name $ROLE_NAME >/dev/null 2>&1; then
    ered "错误： Role创建失败，创建Kubernetes service account and AWS IAM Role失败!!!"
  ...
  if [ -z "${ATTACHED_POLICIES}" ] || [[ "${ATTACHED_POLICIES//\"/}" != "${EXPECT_POLICY}" ]]; then
    red "错误： Policy绑定失败，创建Kubernetes service account and AWS IAM Role失败!!!"
    echo -e "2. 创建Kubernetes service account and AWS IAM Role失败!!!" >>${RESULT_FILE}
    exit 1
  fi
