...
#############################################
EKS_VERSION="1.34"
CLUSTER_NAME="eks-shushu-ta-v1-34"
...
validate_parameters() {
    ...
    # 校验子网 ID
    local subnets=("$PUBLIC_SUBNET_ID1" "$PRIVATE_SUBNET_ID1" "$PUBLIC_SUBNET_ID2" "$PRIVATE_SUBNET_ID2")
    for subnet in "${subnets[@]}"; do
        if [[ -z "$subnet" || "$subnet" == subnet-* ]]; then
            log_error "请配置正确的子网ID"
            exit 1
        fi
    done
