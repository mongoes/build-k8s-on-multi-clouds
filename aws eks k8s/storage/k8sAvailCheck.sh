...
#  - alibaba/huawei : 4池。阿里 ACK 在 spot 资源不足时自动 fallback 到 on-demand, 故无需独立 od-32c128g 池
#  - tencent/volc   : 5池(reserved/od/spot 全谱)
#  - google(GKE)/aws(EKS): 无原生包年包月(reserved)实例,常驻节点运维可能命名为 reserved-* 或 od-*。故 4c32g/32c128g 两档各为"reserved 或 od 二选一"(用 | 分隔的 OR 组,组内任一别名可调度即算该档通过),spot-32c128g 唯一。
...
format_resource() {
    ...
    # m 后缀二义: CPU=毫核(3920m=3.9核) vs 内存=毫字节(火山VKE 把 allocatable.memory
    # 上报为毫字节, 如 29258405314600m = 29258405314.6 字节)。用量级区分:
    # 毫核现实上限约几十万(<1000核), 毫字节最小也在 10^9 量级(1GB=10^12m), 以 10^8 为界。
    if [[ "$suffix" == "m" ]]; then
        if awk "BEGIN{exit !($num > 100000000)}"; then
            # 毫字节 -> 字节 -> Gi (1024进制)
            awk "BEGIN {printf \"%.1fGi\", $num/1000/1024/1024/1024}"
        else
            # 毫核 -> 核
