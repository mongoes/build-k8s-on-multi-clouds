...
# 特殊处理:
#   - 内置K8S(vmware/kvm/qemu/baremetal): 用 local-path,不统一为 te-disk, 仅输出手动指引
...
ensure_storageclass() {
    ...
    # 内置K8S(local-path)不适用此统一逻辑, 跳过以免误摘 local-path 的 default 注解。
...
allowVolumeExpansion: true
EOF
        # te-nfs(EFS文件存储): 依赖AWS控制台返回的文件系统ID(fileSystemId), 无法自动获取, 需用户手动创建
        log_warning "AWS环境还需创建 te-nfs(EFS文件存储)StorageClass，但其依赖控制台返回的文件系统ID(fileSystemId)，无法自动创建"
        log_warning "请在EFS控制台创建文件系统并获取 fileSystemId(形如 fs-xxxxxxxx)后，将下方模版的 fileSystemId 替换为实际值并手动 kubectl apply:"
        cat <<'EOF' | tee -a "$LOG_FILE"
---------------- te-nfs.yaml (请替换 fileSystemId 后手动创建) ----------------
kind: StorageClass
apiVersion: storage.k8s.io/v1
metadata:
  name: te-nfs
...
parameters:
  ...
  fileSystemId: fs-xxxxxxxxxxxxxxxxx   # ← 替换为EFS控制台返回的实际文件系统ID
  directoryPerms: "700"
reclaimPolicy: Retain
-----------------------------------------------------------------------------
EOF
        record_result "AWS te-nfs文件存储SC(需手动创建)" "WARN" "依赖EFS控制台返回的fileSystemId，请获取后按模版手动 kubectl apply"
        ;;
    vmware | kvm | qemu | baremetal)
        # TE内置K8S需要ta-admin操作，仅输出指引(用 local-path, 不统一为 te-disk)
        ...
        log_warning "TE内置K8S环境未检测到local-path-provisioner StorageClass，内置K8S处于特殊维护阶段，请在云原生沟通群确认后手动处理:"
        ...
        log_info "  如需NFS文件存储，请另行手动创建 te-nfs(provisioner: nfs-provisioner)，详见飞书《云厂商CSI安装配置》第3节"
        return 1
        ;;
    *)
...
main() {
    ...
    log_info "4. StorageClass就绪检查"
    log_info "5. Pod部署启动检查(并发探测所有节点池)"
    log_info "6. 节点池与节点配置检查"
    log_info "7. 节点组契约校验(规格/付费类型/污点 vs 池名声明)"
    log_info "8. 本地服务器访问Pod网络连通性检查"
    log_info "9. Pod访问本地服务器网络连通性检查"
    log_info "10. 端到端存储验证(SC->PVC->挂载PVC的Pod起服，CSI就绪以此为唯一金标准)"
    echo ""
    ...
    *aws*)
    ...
    if verify_storage_e2e; then
        record_result "端到端存储验证(SC->PVC->Pod挂载)" "PASS" "PVC动态供给+卷挂载成功，底层CSI存储就绪"
    else
        record_result "端到端存储验证(SC->PVC->Pod挂载)" "WARN" "PVC未Bound或挂载Pod未就绪，使用PVC的应用请排查CSI"
    fi
